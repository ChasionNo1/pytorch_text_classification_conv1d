...
training_data = tt.datasets.IMDB(split='train')
test_data = tt.datasets.IMDB(split='test')
...
collate_fn = lambda y,x: (text_preprocessing(x), label_pipeline(y))

# Create data loaders to batch, shuffle, and map the data.
batch_size = 50
train_dataloader = tc.utils.data.DataLoader(training_data, batch_size=batch_size, shuffle=True, collate_fn=collate_fn)
test_dataloader = tc.utils.data.DataLoader(test_data, batch_size=batch_size, shuffle=True, collate_fn=collate_fn)
...
x = text_preprocessing(hypothetical_review)
